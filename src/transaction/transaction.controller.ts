...
export class TransactionController {
  ...
  @Get(':billerId')
  async findOne(
    @Param() filterDto: FilterTransactionDto,
  ): Promise<Transaction | null> {
    try {
      const { billerId, type, customerIdentifier } = filterDto;
      ...
      if (!transaction) {
        throw new NotFoundException(
          `Transaction not found for billerId: ${billerId}, type: ${type}, customerIdentifier: ${customerIdentifier}`,
        );
      }
      ...
      console.error(
        `Error fetching transaction for billerId ${filterDto.billerId}:`,
        error,
      );
